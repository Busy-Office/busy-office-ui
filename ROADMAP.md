...
5. [ ] **110.5 — generated STATUS.md: the human's ten-second "now" view.**
       From 110.0: the owner's monitoring channels today are chat
       summaries (the channel the number-honesty rules exist to distrust),
       git log, and an 11.6k-line roadmap. Everything a status view needs
       is already structured (loops.db, dispatch_status, checkbox scan).
       **Accept:** a derived, rebuildable STATUS.md (mirrors doctrine)
       regenerated by record_iteration.py so it cannot drift from the
       log — open items by slice, counters, owner-blocked list, last 10
       iterations; one screen, no web surface. **Sunset test written in:**
