...
- [x] Inline edit (.bo-input--seamless, always-a-real-input pattern) + saved-views
...
- [x] Approval workflow: status timeline (.bo-timeline), audit trail (.bo-audit);
      queue + action bar reuse the table/bulk patterns (documented)
- [x] Dashboard: stat tile (.bo-stat, proportional figures + direction×goodness
      deltas), widget grid with per-widget container queries
- [x] Wizard stepper (.bo-stepper, aria-current="step", real ✓ glyphs)
...
- [x] Print/report layer: @page + break control + header repetition + print-only
      utilities + .bo-print-report link URLs + forced status colors; docs page
