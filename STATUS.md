...
Generated at: 2026-08-30 11:51 UTC
...
- **Slice 221** (3 open)
  - 221.1 — pin htmx to 4 where it can actually run: `examples/po-app` first, `apps/docs` behind 221.3.
  - 221.2 — the scoped audit Slice 114 made a precondition, now with its surface counted.
  - 221.3 — BLOCKER, and it needs an owner call: htmx 4 removes `hx-ext`, and the docs shell's head-support extension has no htmx-4 release.
- **Slice —** (1 open)
  - AT runtime evidence
...
```
dispatch status — counter-triggered rules (1215 iterations logged)
