...
| An icon catalogue — the hundreds-of-glyphs set | The small shipped glyph set, then `--bo-icon-src` for anything else: point it at Lucide, Heroicons or your own SVG and `.bo-icon` supplies sizing, `currentColor`, density and forced-colors. Every glyph is an inlined SVG, so cost grows strictly with set size and a 200-icon catalogue costs most of what the rest of the framework costs — `/components/icon` computes the live figures from the shipped stylesheet rather than quoting a snapshot (the two this row used to carry, "12 glyphs" and "10.3%", were stale by 2026-08-24) | 2026-08-19, roadmap 40.1 |
