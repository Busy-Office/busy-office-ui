...
with nothing mechanical behind it: a wake that skips it gets no second signal.

**A conflict does arrive once a wake has RECORDED, which is the last thing it
does.** `record_iteration.py` runs *after* the commit, once per wake, so for
nearly all of a wake the `loop-log.md` append point is untouched; the same wake
that merged clean conflicted on its second rebase, after its rows existed.
Resolving such a conflict: **keep BOTH row sets**, then regenerate the mirrors
...
**Three options were refused, each for a measured reason; the measurements are
in `LOOPS-archive.md`.** A **claim marker committed before working** (a lock in
git) — it must be pushed to be visible, `pages.yml` has no `paths-ignore`, so it
doubles the Pages deploys per wake, and it still races. **Partition by loop
type** — it partitions the loops but not the shared counters, which makes
starvation a design property. **The local session stops dispatching** — Slice
162's own postscript refutes it: the redundant cloud wake found a real defect
the local session had shipped, and rule 1 has to run wherever owner input
lands.
...
4. **Build item queued anywhere in the backlog** — the OLDEST still-open item
   ...
   **This paragraph used to pin the sweep history in numbers, and by 2026-08-28
   ...
   The archive-sweep **cycle table**, its regrowth-per-commit reading, the
   endogeneity caveat and the ratio-versus-denominator correction are in
   **`LOOPS-archive.md`** (roadmap 191.3). They are history a wake needs when
   it ARGUES about the sweep's cadence, not when it dispatches — what a wake
   does here is unchanged by them: read `ROADMAP.md`, and if this rule is
   walking thousands of lines, triage the sweep and run it. Following 167.2,
   the numbers moved and the instruction did not, and the instruction stays a
   paragraph rather than becoming a pointer, because a pointer is read less.
...
`minmax(0, 0fr)` / `minmax(0, 1fr)` in BOTH states — the same track function at
...
- **Batch pushes: one per wake** (2026-08-16, owner-reported unstyled
  ...
  rebuild. Do a full `npm run docs:container` (validates the whole build path
  — clean `npm ci`, the Containerfile itself — and stamps `/build-id.json`
  with the commit and uncommitted build inputs, 377.12) only at checkpoints:
  a slice closing, or after touching the Containerfile/deps.
