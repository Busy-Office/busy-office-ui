...
export function initMoneyField(): void {
  ...
  document.addEventListener('change', (e) => {
    ...
    // applyDecimals routes a data-grouped amount (123.1) through its raw
    // value — the visible text carries separators a plain parse cannot read.
    applyDecimals(amount, decimalsOverride(select, root) ?? currencyDecimals(select.value));
  });
}
